...
static void plane_unlock(struct wlr_eglstream_plane *plane) {
	...
	sem_post(&plane->flip_thread_sem_id);
	pthread_join(plane->flip_thread_id, NULL);
	sem_destroy(&plane->flip_thread_sem_id);
	free(plane);
}
...
static void *flip_handler(void *arg) {
	struct wlr_eglstream_plane *plane = (struct wlr_eglstream_plane*)arg;
	struct wlr_eglstream *egl_stream = &plane->stream;
	struct wlr_egl *egl = egl_stream->egl;
	while (true) {
		if (sem_wait(&plane->flip_thread_sem_id) != 0) {
			break;
		}
		...
		egl->procs.eglStreamConsumerAcquireAttribNV(egl->display, egl_stream->stream,
				acquire_attribs);
	}
...
static struct wlr_eglstream_plane *find_or_create_plane(
	...
	if (plane == NULL) {
		...
		plane->width = width;
		plane->height = height;
		if(sem_init(&plane->flip_thread_sem_id, 0, 0) != 0) {
			wlr_log(WLR_ERROR, "EGLStream semaphore init failed for plane %u", plane_id);
			goto error;
		}
		if (pthread_create(&plane->flip_thread_id, NULL, flip_handler, plane) != 0) {
			wlr_log(WLR_ERROR,
				"EGLStream flip handler creation failed for plane %u", plane_id);
			sem_destroy(&plane->flip_thread_sem_id);
			goto error;
		}
